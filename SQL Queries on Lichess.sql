#The following query shows the players that didn't play in 2013 but played in 2014

/*
We only want the players who have played games as both colours, so we will only use the Black column, given that it contains the 
players who have played as both sides, and White contains players only playing for one side
*/
with combined_usernames_2013 AS(
SELECT 
	DISTINCT m2013_black.Black AS Username     #Ensures that there are no duplicates in the results
FROM 
	merged_2013_shortened AS m2013_black
JOIN 
	merged_2013_shortened AS m2013_white       #Doing a self join to get the people who played as both black and white
ON 
	m2013_black.Black = m2013_white.White
WHERE 
	m2013_black.Black IS NOT NULL
)
,
-- Same query as above but with the 2014 dataset
combined_usernames_2014 AS(
SELECT 
	DISTINCT m2014_black.Black AS Username
FROM 
	merged_2014_shortened AS m2014_black
JOIN 
	merged_2014_shortened AS m2014_white
ON 
	m2014_black.Black = m2014_white.White
WHERE 
	m2014_black.Black IS NOT NULL
)
    

#cu stands for combined usernames
SELECT 
	DISTINCT cu2014.Username       #Ensures that there are no duplicates in the results
FROM 
...
WHERE 
	cu2013.Username IS NULL    #Username will be NULL if they played in 2014 but not 2013
;
...
SELECT 
	White, 
    CAST(num_wins_white AS DECIMAL(10, 2)) / num_games_white AS win_percentage,
	num_games_white, 
    num_wins_white
FROM 
	white_summary 
...
#because some had ratings which were significantly higher for one colour than the other
	...
	SELECT 
		hwr.White AS Player, 
		(AVG(hwr.max_white) + AVG(hbr.max_black)) / 2 AS Avg_Highest_Rating, 
        max_white, 
        max_black 
	FROM 
